...
IF OBJECT_ID('dbo.Healthcheck') IS NULL
  EXEC ('CREATE PROCEDURE dbo.Healthcheck AS RETURN 0;')
GO

ALTER PROCEDURE [dbo].[Healthcheck]
...
AS
	...
	SELECT @Version = 1.1, @VersionDate = '20150201'
	...
	Known limitations of this version:
	 ...
	 - If you have offline databases, Healthcheck fails the first time you run it,
	...
	*/'
	...
	BEGIN
		...
		*/
		...
		want to run. Then, when you run Healthcheck, you can specify these parameters:
		...
		IF ( ( SERVERPROPERTY('ServerName') NOT IN ( SELECT ServerName
			...
			BEGIN
				...
				*/
				...
				We don't include that as part of Healthcheck, of course, because
		...
		WHERE   is_percent_growth = 1 ';
				...
				IF EXISTS ( SELECT  *
					...
					IF NOT EXISTS ( SELECT 1
                        ...
                        /* Outdated Healthcheck - Healthcheck is Over 6 Months Old */
                        IF NOT EXISTS ( SELECT  1
				                        FROM    #SkipChecks
	                        ...
	                        BEGIN
		                        INSERT  INTO #HealthcheckResults
				                        ...
				                        SELECT 155 AS CheckID ,
						                        0 AS Priority ,
						                        'Outdated Healthcheck' AS FindingsGroup ,
						                        'Healthcheck is Over 6 Months Old' AS Finding ,
						                        'http://www.BrentOzar.com/blitz/' AS URL ,
						                        'Some things get better with age, like fine wine and your T-SQL. However, Healthcheck is not one of those things - time to go download the current one.' AS Details
	                        END
		  ...
		  from [?].INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_DEFINITION LIKE N''%WITH RECOMPILE%'' AND SPECIFIC_NAME NOT LIKE ''Healthcheck%%'';';
							END
	...
	/*Verify that the servername is set */
		...
		/*Check for collation conflicts between user databases and tempdb */
			...
			FROM sys.dm_os_sys_info';
						...
						IF NOT EXISTS ( SELECT  1
							...
							BEGIN
								...
								IF NOT EXISTS (SELECT * FROM #HealthcheckResults WHERE CheckID = 152)
								BEGIN
									INSERT  INTO #HealthcheckResults
											( CheckID ,
											  Priority ,
			...
			END /* IF ( ( SERVERPROPERTY('ServerName') NOT IN ( SELECT ServerName */
				...
				VALUES  ( -1 ,
						  0 ,
						  'Healthcheck (TM) v' + CAST(@Version AS VARCHAR(20)) + ' as of ' + CAST(CONVERT(DATETIME, @VersionDate, 102) AS VARCHAR(100)),
						  'From Brent Ozar Unlimited' ,
						  'http://www.BrentOzar.com/blitz/' ,
						  'Thanks from the Brent Ozar Unlimited team.  We hope you found this tool useful, and if you need help relieving your SQL Server pains, email us at Help@BrentOzar.com.'
				...
				IF @EmailRecipients IS NOT NULL
					...
					SET @StringToExecute = 'SET NOCOUNT ON;SELECT [Priority] , [FindingsGroup] , [Finding] , [DatabaseName] , [URL] ,  [Details] , CheckID FROM ##HealthcheckResults ORDER BY Priority , FindingsGroup, Finding, Details; SET NOCOUNT OFF;';
					SET @EmailSubject = 'Healthcheck (TM) Results for ' + @@SERVERNAME;
					SET @EmailBody = 'Healthcheck (TM) v' + CAST(@Version AS VARCHAR(20)) + ' as of ' + CAST(CONVERT(DATETIME, @VersionDate, 102) AS VARCHAR(100)) + '. From Brent Ozar Unlimited: http://www.BrentOzar.com/blitz/';
					IF @EmailProfile IS NULL
						EXEC msdb.dbo.sp_send_dbmail
							@recipients = @EmailRecipients,
							@subject = @EmailSubject,
							@body = @EmailBody,
							@query_attachment_filename = 'Healthcheck-Results.csv',
				...
				IF @OutputDatabaseName IS NOT NULL
					...
					BEGIN
						...
						SET @StringToExecute = N' IF EXISTS(SELECT * FROM '
							...
							+ ', CheckID, DatabaseName, Priority, FindingsGroup, Finding, URL, Details, QueryPlan, QueryPlanFiltered FROM #HealthcheckResults ORDER BY Priority , FindingsGroup , Finding , Details';
						EXEC(@StringToExecute);
					END
